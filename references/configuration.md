...
`benchmark_review_only` 不是另一种长期基准配置，也不是跳过基准的开关。它由 `products adopt` 在已有 `summary` 页发现活跃但来源未证明的基准/超额列时临时使用，也适用于指数技术映射已经证明、但当前机构用途的数据许可尚未确认的情况。审查预览不能批准为自动更新；只有技术来源验证与使用许可确认分别成立后，才能移除该字段并配置 `benchmark`。如果指数身份最终无法证明，应在副本中与用户确认业务口径后改用真正无基准的产品页；如果只是许可待确认，则由数据负责人、合规人员或用户提供已授权来源或批准依据，不把公网可访问性当作授权。
