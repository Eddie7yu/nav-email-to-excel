...
每条路由必须按目标工作表的实际结构选择模式。模式不会自动互相降级，AI 也不得为了绕过验证错误而擅自切换。

| `sheet_mode` | 适用结构 | 历史要求 | 汇总行 |
| --- | --- | --- | --- |
| `summary`（默认） | 已有历史数据、固定汇总行和受管理公式的复杂表 | 至少匹配 `minimum_history_dates` 个不同日期，默认 2 个 | 数据区后必须紧跟可识别汇总行 |
| `append` | 空白、只有表头，或没有汇总行的简单追加表 | 允许历史不足的冷启动，但验证报告会显示警告 | 不需要；数据区下方不得有页脚内容 |

`append` 模式至少配置非空的 `code` 或 `product_name`，并要求目标表中存在对应的可写列。产品名称和代码只作为经过用户确认的路由常量写入，不从邮件模糊猜测。空白工作表会在预览中初始化标准表头；目标工作簿和目标工作表仍必须提前存在。首次能可靠落表的最低记录是：
...
推荐把公式或结果放在 navctl 未托管的独立分析工作表，或由用户另行批准生成的分析文件。预览完整性检查会保护未托管工作表不被 navctl 改动，但固定范围公式未必会随新增行自动扩展；使用整列引用、动态命名范围或 Excel 表格前仍需人工检查。不得把自定义分析公式塞进受管工作表的汇总行，也不得把“批准写入净值”解释成“批准修改分析模型”。
...
`validation.minimum_history_dates` 不得低于 `2`，并作为 `summary` 模式的强制历史门槛。`append` 模式达不到该数量时会标记为冷启动并产生可见警告，而不是伪装成已经完成历史核验。`max_future_days` 用于阻止未来日期，`max_period_change` 用于在创建预览前拦截不合理的单位净值跳变。只能依据有记录的产品行为调整这些值，不能为了让失败的运行通过而放宽。
