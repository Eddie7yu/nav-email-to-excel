...
部署后先运行 `navctl.py products sync`，用一次邮箱候选扫描比较当前路由和工作簿。报告包含：
...
正确处理顺序是：先读取脱敏路由诊断并用完全虚构样例复现；属于上述明确未配置行时使用内置选择式路由；属于通用 HTML/附件格式时完善 `parser: auto`；只有供应商专有格式才在本地 `app/parsers/` 增加受审阅解析器。无论哪种解析方式，最终都继续走 `navctl.py preview`、用户首次批准、`commit` 和正式计划任务，不得另建直接写表入口。
