...
数组公式按范围分类：数据区数组、多单元格数组、跨越插入点的数组和最终可提交预览中需要移动的数组一律失败关闭。仅 `benchmark_review_only` 的不可提交审查副本允许一个窄例外：数组范围只有自身一个单元格，且该单元格位于受管汇总行的基准点位、基准收益或超额列；程序在插行前清空副本中的公式，在新汇总行记录 `review_array_formulas_cleared`，正式工作簿不变。解决基准来源后，这个例外自动失效；Agent 必须先在受保护副本中用 Excel/WPS COM 或可证明等价的方法重建公式，核对公式作用域和计算缓存值，再生成最终完整预览。不得把审查副本直接提交。
...
能由权威指数定义、工作簿历史日期与数值唯一对应到持续来源时，Agent 应自动配置并验证，不把内部映射工作交给用户。工作簿使用自定义基准名称，且本地资料和公开权威来源都无法证明唯一对应关系时，属于真正的业务歧义：只暂停该页的基准提交并询问用户确认业务身份或来源，不得用名称相似度猜指数，也不得拖住其他已唯一核实产品的预览。
