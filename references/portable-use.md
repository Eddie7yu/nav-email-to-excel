...
授权码保存后，优先运行 `navctl.py propose --headers-only --header-limit 25`，只读 From/Subject 并与工作簿身份比较；范围能唯一确定时，再用 `propose --sender ... --subject-contains ...` 只下载精确范围的完整邮件。头部信息不足时可以扩大有界样本，或在产品只存在于正文/附件时有依据地使用普通 propose，不要另写邮箱诊断脚本。自动发现产品代码、主题规律和到达时间；已有表再读取工作簿完成路由、列映射和口径，新建表则从邮件规律判断日频/周频，只有明确资料证明基准时才使用有指数模板。不要让我手工抄发件人邮箱、产品代码或逐项填写 AI 能从本地证据确定的信息；只有真正歧义才问我。同一封托管邮件包含多个产品时，只选择已配置代码，忽略有明确代码的其他产品；无代码、冲突或解析失败仍停止。根据邮件历史给我一个运行安排建议，并明确询问我希望的更新频率、运行星期和本地时间，把确认结果写入 schedule；Windows 正式部署不得省略定时任务。

部署后我新增产品时只需表达两种意图：如果我已经建立目标 Sheet，默认运行 `products adopt` 自动识别并接管；如果我说“格式照某个 Sheet”，默认运行 `products clone` 先备份，再通过 Excel/WPS 完整复制格式、清空参考产品内容并接管。这限制的是我需要提供的信息，不限制你为特殊工作簿采用本地兼容手段。不要让我填写发件人、代码、`summary/append` 或手工修改 config。新增后重新生成一次预览让我检查，原计划任务继续使用。

按表头含义识别列；只要邮箱能唯一识别产品就补入真实历史，但必须完整保留 Excel 模板。原表是周度就按周度写，不得把日度邮件全部灌入；不得清空旧历史、删除公式/累计行/基准列，或为通过验证把 summary 改成 append。若工作表是“唯一一条产品标识+预留日期、净值为空的行，下一行是累计/合计”，自动用最早真实邮件替换并补全该行，再在累计行前插入其余记录，不要问我删行或补造历史数据。如果邮箱回看期只有表内最新日期之后的新数据，只有在工作簿与邮件代码精确一致、主题过滤片段也包含该代码、双方各至少两期、全部严格向后且无冲突时，才生成带警告的首次预览；否则继续失败关闭。把 references/template-preservation-case.md 作为已验证参考，不要把不同前提的新问题强行套入案例。成功且仍适用的完整 propose 结果直接用于配置；只有假设或输入变化时才有目的地重新扫描。配置完成后默认直接运行 preview，让它用一次邮箱扫描完成发现、验证和预览。第一次生成预览给我检查；如果当前零新增，生成中文基线报告并在我批准后启用自动更新，不得等待下一封邮件。标准流程报错时可以自主决定诊断顺序并在 app/diagnostics 编写临时原型，但原型只操作副本，不能直接覆盖正式表或安装为任务；确认方案后接回通用/本地解析器和 navctl 验收链路。我批准预览或基线后，按确认的频率和时间安装自动更新任务。后续每次必须先验证、备份，并在失败时保持正式表不变。
