...
```text
请完整阅读 SKILL.md 和 references/configuration.md，并使用仓库自带脚本，为我的现有工作簿创建一套净值邮件自动化。如果我不知道 IMAP 服务器或认证方式，先阅读 references/email-providers.md，按现有客户端、单位管理员或服务商官方文档核验；不要凭邮箱后缀、MX 记录或 imap.公司域名猜测。创建运行目录后，先运行完全虚构的 navctl 离线演练；演练通过后再接入真实邮箱和工作簿配置。
...
只配置我授权的发件人和工作表。按表头含义识别列；summary 路由至少核对两个不同的历史日期，append 冷启动必须显示历史不足警告并让我重点检查产品标识、日期、净值和表头；产品、日期或数值存在歧义或冲突时必须停止。先生成预览和脱敏验收报告，只有我检查并批准后，才能通过受控的 Excel/WPS COM 流程正式写入。夏普比率、最大回撤等分析应基于核实后的序列，在未托管分析页或单独文件中完成，并先向我确认计算口径。
```
