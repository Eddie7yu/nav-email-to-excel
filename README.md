...
- 扫描本地邮箱中的可解析净值邮件，自动提出“发件人/产品 → 工作表”路由；已有表保持原样，没有表则从内置脱敏模板新建。
...
- 严格汇总模式通常先用至少两个历史日期验证产品代码、日期和净值；若表内只有一条“产品标识+日期、净值为空”的预留数据行，程序会用至少两期邮件历史安全冷启动并保留原汇总行。追加模式也允许无历史冷启动，但必须有产品名称或代码，并显示警告供人工检查。
- 只追加工作簿尾部的新日期；发现历史内部缺口、重复日期或同日冲突时停止。
- 邮箱提供真实历史，现有 Excel 决定落表结构和数据频率：运行时会从既有日期规律或明确表头识别日度/周度行，并拒绝与模板冲突的强制设置；周度模板不会被日度邮件改造成日度空壳，原历史、公式、汇总行、基准和样式必须保留。
...
- 邮箱候选扫描、发件人二次核对、批量邮件只选择已配置产品、产品代码精确匹配、停更路由显式暂停和陈旧数据拦截；
...
它不能保证从未见过的邮件格式都自动兼容。AI 应先利用邮箱历史、工作簿和本地解析器解决；只有仍无法唯一判断时才向用户说明问题。如需新增特殊解析器，可使用运行目录 `app/parsers/` 中的扩展点并先用完全脱敏样例回归，绝不能从邮件或附件加载代码。
...
5. 没有现成工作簿时，AI 核实日/周频率和基准后运行 `navctl.py workbook init-template`；有现成表时跳过，绝不套模板。然后完成 `doctor` 并直接运行第一次 `preview`，不重复串行执行 `discover`、`validate`。
...
部署后新增产品只需要告诉 AI 两件事之一：①“我已经建好这个 Sheet，请接管”，AI 使用 `products adopt` 自动识别表头、频率和邮箱产品；②“这个新产品格式照某个现有 Sheet”，AI 使用 `products clone` 通过 Excel/WPS 完整复制格式、清空参考产品内容并接管。两种方式都不需要重新部署、重新输入授权码或重装定时任务，只需检查一次新产品预览。完整边界见 [SKILL.md 的新增产品章节](SKILL.md#新增产品只走两条路径)。
