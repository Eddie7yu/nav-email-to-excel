...
- 依赖版本锁定、隔离虚拟环境和 Unicode 路径；
- 邮件发件人二次核对、产品代码精确匹配和陈旧数据拦截；
- 累计净值口径、序列重置、日/周收益和基准对齐；
- 未来日期、异常净值跳变、重复写入和历史缺口保护；
- 公式、格式、工作表结构、预览完整性及正式文件不变性检查；
- 本地密钥、日志、预览、备份的隔离与公开仓库脱敏审计。

它不能保证从未见过的邮件格式都自动兼容，也不能替代对业务口径的人工确认。遇到无法证明的情况，正确行为是停止并给出原因；如确需新增特殊解析器，应只在本地开发，并用完全脱敏的样例做回归测试后再考虑公开。

## 适用范围与限制

- 邮箱：IMAP over SSL，使用应用专用密码或授权码；暂不支持仅限 OAuth 的登录。
- 附件：Excel、CSV 和文本型 PDF；扫描 PDF 暂不提供 OCR。
- 预览：Windows、Linux、macOS 均可运行 Python 流程；当前持续集成覆盖 Windows 与 Linux。
- 正式写入：仅支持 Windows，并要求 Microsoft Excel 或 WPS 表格的 COM 接口可用。
- 定时任务：仅支持 Windows 登录态，且只生成预览。
- 工作簿：面向已有 `.xlsx`/`.xlsm`；以现有表头、公式和样式为准。
...
4. 用户亲自在本机运行 `navctl.py secret set`，隐藏输入邮箱授权码。
5. AI 在本机发现工作簿结构，并根据用户确认的信息配置路由和业务口径。
6. 依次执行 `doctor`、`discover`、`validate`、`preview`。
7. 用户检查预览中的工作表、新增行、公式和格式。
8. 只有用户明确批准后，Windows 才执行 `commit --yes-reviewed-preview`；否则正式工作簿保持不变。
