...
这是一个面向中文用户和本地 AI Agent 的公开 Skill：用户只需在本机输入一次邮箱授权码，AI 就能发现净值发件人、匹配产品与工作表、完成第一次预览验收，并在之后自动备份和更新 Excel。
...
- 扫描本地邮箱中的可解析净值邮件，自动提出“发件人/产品 → 工作表”路由；也能初始化空白目标工作表。
...
它不能保证从未见过的邮件格式都自动兼容。AI 应先利用邮箱历史、工作簿和本地解析器解决；只有仍无法唯一判断时才向用户说明问题。如需新增特殊解析器，可使用运行目录中的 `parsers/` 扩展点并先用完全脱敏样例回归，绝不能从邮件或附件加载代码。
...
- 工作簿：面向已有 `.xlsx`/`.xlsm`，目标工作表必须已存在。默认 `summary` 严格模式要求受管数据区后紧跟可识别的汇总行，也支持“唯一预留数据行只有产品标识和最早日期、净值为空”的安全冷启动，累计/合计行无需删除。显式 `append` 模式支持空白、只有表头或无汇总行的简单追加表。两种模式不会自动互相降级；追加表的数据区下方不得有页脚内容，复杂公式应放在未托管分析页或单独文件。
...
```text
请使用 $nav-email-to-excel，为这个工作簿部署净值邮件自动化。不要让我手工提供发件人邮箱；我输入授权码后请自动发现和配置，第一次给我检查，之后自动备份更新：<工作簿绝对路径>
```
...
2. AI 获取工作簿路径和收件邮箱账号；IMAP 设置优先从现有客户端或服务商资料中核实。
3. AI 运行 `navctl.py secret launch` 主动弹出可见窗口；用户只需在该窗口隐藏粘贴一次授权码，AI 随后用 `secret status` 检查，不要求用户输入终端命令。
4. AI 运行 `navctl.py propose`，从邮箱自动发现发件人、产品代码、主题与到达时间，结合工作簿生成路由和写表配置；随后给出运行时间建议，并询问用户希望每周一次、工作日每天或哪些星期、几点自动更新。
5. AI 完成 `doctor` 后直接运行第一次 `preview`；该命令用一次邮箱扫描完成发现、历史验证和预览，不重复串行执行 `discover`、`validate`；只有真正歧义才询问用户。
...
3. 以新生成的 `config.json` 为基础，让 AI 迁移并复核 `routes`、`column_overrides`、`style`、`schedule`、`validation` 和 `retention`。不要用旧文件整体覆盖，以免带回旧的结构版本或 `runtime_id`。
