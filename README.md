...
这是一个面向中文用户和本地 AI Agent 的公开 Skill：它把“从 IMAP 邮箱读取净值邮件、核对或初始化 Excel 数据、生成预览、经人工批准后受控写入”的流程固化成可验证的脚本。
...
- 把发件人和产品精确映射到已有工作表，不靠模糊猜测；也能在明确启用追加模式后初始化空白目标工作表。
...
6. AI 在本机发现工作簿结构，根据用户确认的信息选择严格汇总或简单追加模式，并配置路由和业务口径；不得因验证失败自动降级为追加模式。
...
需要夏普比率、最大回撤、年化收益等专业指标时，不要把净值写入和分析口径混在一起。先由本工具生成经核验的净值序列，再让 AI 在未托管分析工作表或单独文件中计算，并由用户确认收益频率、年化因子、无风险利率、样本区间和缺失值处理。详见[配置说明中的“派生专业指标”](references/configuration.md#派生专业指标)。
