...
description: 部署、配置、验证、运行或修复本地 IMAP 邮箱到 Excel 的基金净值自动化。让 AI 在用户输入一次邮箱授权码后自动发现发件人和产品；已有工作簿则原样保留，没有工作簿则从内置脱敏模板创建；完成首次预览验收后，在 Windows 上通过 Excel/WPS 自动备份和持续写入。支持正文及 Excel/CSV/PDF 附件、日度/周度表、基准超额、简单追加表、复杂汇总表和本地解析器。
---
