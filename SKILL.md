...
普通用户只需要用自然语言说明“我已经建好一个 Sheet”或“新产品照哪个 Sheet 的格式”。不得让用户选择 `summary/append`、填写发件人、抄产品代码或编辑 JSON。AI 先运行一次 `products sync`，用邮箱候选、产品代码、工作簿表头和历史日期唯一确定 `proposal_index`；本次候选报告仍然新鲜时使用 `--use-existing-proposals`，不要重复登录邮箱。
...
标准流程出现解析或路由错误时，AI 固定按“读取 `route-report.json`/`validation-report.json` → 用完全脱敏样例复现 → 能通用修复则修复 `parser: auto`，确属特殊格式才使用受审阅的本地解析器 → 重新执行 `preview`”处理。不得另建 `nav_sync.py`、硬编码发件人/代码/Sheet/列号、直接用 openpyxl 或 COM 覆盖正式表，也不得把这种脚本安装为计划任务；短脚本绕开的预览签名、批准、并发锁、备份和临时副本核验都是正式流程的一部分。
...
`preview` 内部已经依次执行邮件发现和历史验证，并在 `app/` 写出 `route-report.json`、`validation-report.json`，在根目录 `previews/` 写出预览结果；有待写入日期时生成预览工作簿，没有新增日期时生成中文零新增基线验收报告。正常首次部署不得再预先单独运行 `discover` 和 `validate`，否则同一批邮件会被重复登录、重复扫描。只有定位某一阶段错误且不需要生成预览时，才把这两个命令作为诊断工具单独使用。
