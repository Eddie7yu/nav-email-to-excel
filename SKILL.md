...
有新增数据时，预览是本地工作簿副本。已有表必须与原表并排检查：工作表列表、原有历史、日期频率、表头、公式数量与范围、汇总行、基准列和格式都应保留。新建表重点检查产品标识、日/周频率、是否应有基准、首批真实日期、净值、日/周收益和超额公式，以及等线 10 号、浅蓝表头、收益浅黄、超额亮黄、红涨绿跌。零新增时检查基线报告中的产品页、频率、已核验历史数量、邮箱与工作簿最新日期及待写入数量。不要直接修改正式工作簿。
