...
预览是本地副本。已有表必须与原表并排检查：工作表列表、原有历史、日期频率、表头、公式数量与范围、汇总行、基准列和格式都应保留。普通公式按文本核验；不受插行影响的数组公式按公式文本和作用范围核验，不能因 openpyxl 对象身份不同误报改变。受管页中会被移动、跨越插入点或位于模板复制行的数组公式必须停止预览，因为 openpyxl 不能可靠平移其作用范围。新建表重点检查产品标识、日/周频率、是否应有基准、首批真实日期、净值、日/周收益和超额公式，以及等线 10 号、浅蓝表头、收益浅黄、超额亮黄、红涨绿跌。不要直接修改正式工作簿。
