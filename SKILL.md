...
description: 部署、配置、验证、运行、扩展或修复本地 IMAP 邮箱到 Excel 的基金净值自动化。用于首次安装、邮箱与产品发现、预览验收、持续写入，以及新增、暂停或恢复产品；已有工作簿原样保留，没有工作簿才从内置脱敏模板创建。支持接管用户已建产品 Sheet、照受管 Sheet 完整复制格式新增产品、正文及 Excel/CSV/PDF 附件、日度/周度表、基准超额、简单追加表、复杂汇总表和本地解析器。
---
...
路由已经配置后，共享托管发件人的邮件必须先批量读取最小邮件头，并按各路由的 `subject_contains` 过滤，再计算 `max_messages`、读取大小和下载正文/附件。`max_header_messages` 对过滤前的邮件头集合设置独立资源边界，超限时要求缩短回看窗口或经核实后调高，不能静默截断。这样数量限制约束的是实际受管产品邮件，而不是该托管发件人的全部历史。没有配置主题过滤的路由仍视为授权该发件人的全部范围，不能静默跳过无法解析的邮件。
